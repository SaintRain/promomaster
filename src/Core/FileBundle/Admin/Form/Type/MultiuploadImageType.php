...
class MultiuploadImageType extends AbstractType
{
    ...
    {
        $this->options['core_file'] = $options['image'];
        $this->logic                = $logic;
    }
        ...
        $options['parent_form']->getFormBuilder()->addEventListener(FormEvents::PRE_BIND,
            ...
            if (!empty($formData)) {
                ...
                if (isset($formData[$fieldName])) {
                    ...
                    //$this->logic->clearTableAndRemoveFile($obj, $data);
                }
            }
        });
    }
    ...
    public function buildView(FormView $view, FormInterface $form, array $options)
    {
        ...
        if ((isset($view->vars['attr']['multiple']) && $view->vars['attr']['multiple'] !== true)) {// || $options['is_main'] === true) {
        ...
        $view->vars['is_main']             = $options['is_main'];
        $view->vars['hide_field']          = $options['hide_field'];
        $view->vars['width']               = $options['width'];
        if (!isset($options['google_api']['use'])) {
            $options['google_api']['use'] = true;
        }
        if (!isset($options['google_api']['search_field'])) {
            $options['google_api']['search_field'] = 'captionRu';
        }
        $view->vars['google_api'] = $options['google_api'];
    }
