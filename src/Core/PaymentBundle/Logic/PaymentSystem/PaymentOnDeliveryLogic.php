...
/**
 * Сервис для работы с платежной системой PaymentOnDelivery (наложный платеж)
 *
 * @author Sergeev A.M.
...
class PaymentOnDeliveryLogic extends PaymentSystemLogic
{

    const PAYMENT_SYSTEM = 'PaymentOnDelivery';
     ...
     * @return type
     */
    public function getRequestURL(Payment $payment, $options)
    {
        $URL = $this->container->get('router')->generate('core_order_finish_with_payment_id', ['paymentSystem' => self::PAYMENT_SYSTEM, 'id' => $payment->getId()]);
    ...
    public function getResultMessagesAndPayment($request)
    {
        ...
        if (null === $payment) {
            ...
            $respose['success'][] = $this->trans('success.payment_on_delivery');
        }
