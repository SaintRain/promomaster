...
class GagController extends Controller
{
    ...
    public function listAction($page = 1)
    {
        ...
        if ($url = $this->get('core_banner_logic')->getRedirectUrlIfPageEqualOne('core_cabinet_gag_list_first_page')) {
            return new RedirectResponse($url, 301);
        }

        $banners = $this->get('core_banner_logic')->getGagsInCabinetForPage($page);

        return $this->render('CoreBannerBundle:Gag\Cabinet:list.html.twig', ['banners' => $banners]);
    }
    ...
    public function editAction($id)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if (!$isBadName && $isValidFile && $form->isValid()) {
                ...
                return new RedirectResponse($this->generateUrl('core_cabinet_gag_edit', ['id' => $id]));
            } else {
                return $this->render('CoreBannerBundle:Gag\Cabinet:edit.html.twig', ['type' => $type, 'banner' => $banner, 'form' => $form->createView()]);
            }
        } else {
            return $this->render('CoreBannerBundle:Gag\Cabinet:edit.html.twig', ['type' => $type, 'banner' => $banner, 'form' => $form->createView()]);
        }
    ...
    public function createAction()
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if (!$isBadName && $isValidFile && $form->isValid()) {
                ...
                $this->setFlash('edit_success', 'Новая заглушка добавлена.');

                return new RedirectResponse($this->generateUrl('core_cabinet_gag_edit', ['id' => $banner->getId()]));
            } else {
                return $this->render('CoreBannerBundle:Gag\Cabinet:edit.html.twig',
                    [
                        'type' => $type,
                        'banner' => $banner,
    ...
    public function deleteAction($id)
    {
        ...
        try {
            ...
            $msg = "Невозможно удалить заглушку «{$banner->getName()}», т.к. он задействована в системе на данный момент.";
            $this->setFlash('edit_errors', $msg);
        }

        return new RedirectResponse($this->generateUrl('core_cabinet_gag_list'));
    }
